...
### Scoring-integrity pass (2026-07-03) — user-reported live bugs

League members reported: own goals deducted from the wrong team (one missing for two weeks), Turkey never eliminated, the goal race ordering "alphabetical", and distrust of the bonus math. Root causes found in code and fixed in BOTH `supabase/functions/sync-scores/index.ts` and the client mirror (`src/lib/worldCupApi.ts`):

1. **Own goals** — `parseDiscipline` attributed the own goal to the team the ESPN scoring play credits, i.e. the side that *benefits*. The −1 now lands on the opponents of the credited side. Regression tests pin it.
2. **Group eliminations** — elimination only ever happened to knockout-tie losers. Now, once the full round-of-32 field is known (32 distinct participants), every team not in a knockout tie flips to `eliminated` ("Out at the group stage"). This also corrects the "alive" tiebreak in ranks.
3. **Goal-race ties** — the +10 leader was `sorted[0]`, which on a tie resolves by insertion order (≈ alphabetical). Joint leaders are now a set: every joint leader's backers get the +10 (client leaderboard + snapshot job), and Overview/Matches/My Entry copy says "joint top" honestly.
4. **Breakdown coherence** — `matchPoints` includes per-match bonuses, but the UI listed them *alongside* it, so items never summed to the headline total. New shared `getTeamPointsBreakdown` subtracts the itemised bonuses from a "Results" line; a test asserts items sum to `points`. The `CountrySheet` now carries the breakdown + match-by-match ledger, so ANY country's points are auditable from Matches.
5. **Pot-gap upset (+2/+1)** — verified it IS being applied (both bonuses, visible as "Underdog bonus"). Removing it mid-tournament is a Declan decision; `scripts/audit-scoring.mjs` §5 prints exactly what would change.
...
Two jobs for the last 48 hours. (1) The England–France third-place playoff scores **like a normal knockout match** — the win, clean sheet, goals (which feed the +10 race) and discipline all count — but it grants **no advancement bonus** (the "reached the final" +10 / champion +15), because losing your semi and winning the third-place game does not put you through. Handled in `sync-scores` and the client mirror (`worldCupApi`, `App`, `KnockoutBracket`, `LiveScreen`) via keyword detection ("3rd place"/"bronze"/"play-off for third") plus an explicit `THIRD_PLACE_MATCH_IDS` override. A `thirdPlace` flag on `WorldCupFixture` gates only the advancement effects: `markStage`, champion crowning and elimination are skipped, everything else scores normally. The match is excluded from the Road-to-the-final bracket (it's not a progression tie) and shows in the match centre labelled "3rd place" with a "result and goals count, but no bonus for reaching the final" note. Critical safety net: ESPN labels this match "3rd Place Final", and the old `stageFromSeason` would read the trailing "Final" and crown its winner World Cup champion; third-place is now detected before the "final" catch AND the champion/elimination guards key off the `thirdPlace` flag, so it can never crown a champion even if an id override is applied to a match ESPN labelled plain "Final". `audit-scoring.mjs` §0 lists every "final"-stage match and whether it's detected as third-place, so the behaviour is verifiable against real ESPN data before deploy.
