...
Two jobs for the last 48 hours. (1) The England–France third-place playoff is a **goals-only** match: its goals still feed the highest-scoring-team (+10) race, but it awards no PickFour points, moves no tables, and can never crown a champion. Handled in `sync-scores` and the client mirror (`worldCupApi`, `matchImpact`, `KnockoutBracket`, `LiveScreen`) via keyword detection ("3rd place"/"bronze"/"play-off for third") plus an explicit `GOALS_ONLY_MATCH_IDS` override. A `goalsOnly` flag on `WorldCupFixture` drives the scoring split: only `goalsFor`/`goalsAgainst` accrue; the match is excluded from the Road-to-the-final bracket and shows in the match centre labelled "3rd place · goals count, no PickFour points". Critical safety net: ESPN labels this match "3rd Place Final", and the old `stageFromSeason` would read the trailing "Final" and crown its winner World Cup champion (+15, early +10 banking, wrong showcase); third-place is now detected before the "final" catch and mapped to `semi_final`, so it can never crown a champion even if the id/keyword drop is bypassed. `audit-scoring.mjs` §0 lists every "final"-stage match and whether it's detected as goals-only, so the behaviour is verifiable against real ESPN data before deploy.
