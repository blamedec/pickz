...
| (HEAD) | Editorial hierarchy pass | Overview restructured as a guided feed: hero carries live status copy; the 4-stat grid is **removed** and replaced by a narrative **story strip** ("Tonight's pressure" / "Biggest swing" / "Your next chance" or "+10 goal race" — all computed from live data, each tapping through to its screen); "Matches that matter" promoted directly under the strip with a full-width **lead match card**. My Entry gains a **receipt line** ("21 from your four countries + 0 bonus = 21 points · Latest: ESP 2-0 KSA +6"). Table gains **tie markers** (`=2`) and **rival twins** detection (identical four picks + bonus → called out on the row and explained in the drawer). Stat abbreviations humanised on entry cards and watch cards (Goals/Conceded/Clean sheets/Reds). QA harness gains **`--real` mode**: runs against the real backend strictly read-only (league-api limited to list/get actions, Supabase REST limited to GET, everything else aborted) so layout can be checked against real names, dates, league size and empty states |
